            ...
            if int(w[i + 1]) == 0:  # 当结束的值是空值时
                if int(w[i + 2]) == 1:
                    ...
                    if not y[i]:
                        ry.append('off')
                    tempz = []
    return rx, ry, rz, ru, rt, rc_s, rd_s, rb_s, rc_e, rd_e, rb_e, rp, rzz, rv


# 输出结果
def outputData(data):
    df = pd.DataFrame(columns=(
    'wifitype', 'mac', '开关机状态', '加热开始时间', '加热结束时间', '运行功率', '加热时长', '用电量', '开始水温', '结束水温', '开始目标温度', '结束目标温度',
    '开始变频标识', '结束变频标识', '体积'))
    for key in data:
        kx, ky, kz, ku, kt, kc_s, kd_s, kb_s, kc_e, kd_e, kb_e, kp, kzz, kv = changeData(data[key]['x'], data[key]['y'],
        ...
        df1 = pd.DataFrame({'wifitype': kx, 'mac': key, '开关机状态': ky, '加热开始时间': kzz,
                            '加热结束时间': kz, '运行功率': ku, '加热时长': kt, '用电量': kp, '开始水温': kc_s, '结束水温': kc_e, '开始目标温度': kd_s,
                            '结束目标温度': kd_e,
                            '开始变频标识': kb_s, '结束变频标识': kb_e, '体积': kv})
        df = pd.concat([df, df1])
        ...
        df['每天用电量'] = df.groupby(df['日期'])['用电量'].transform('sum')
        ...
        Line(init_opts=opts.InitOpts(width="100%", height="600px"))
            ...
            .add_yaxis(series_name="加热状态",
                       ...
                       # )
                                   ...
                                   linestyle_opts=opts.LineStyleOpts(width=3, type_="dashed"))
            .set_global_opts(title_opts=opts.TitleOpts(title="每天温度及加热变化(100表示开机；90表示加热)", pos_right="35%"),  # 设置标题
