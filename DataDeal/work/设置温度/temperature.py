...
#非零冷水
    # sql_getall =  '''select mac_id,index_value,pdate FROM u_ads_wh.v_opn_washer_water_rep_mon where prod_group_cd ='FB' and index_code ='0118OB0104'
    # and device_model_desc in('JSQ31-16CQ1BDU1','JSQ25-13JM6(12T)U1','JSQ30-16JM6(12T)U1','JSQ31-16JM6(12T)U1')
    #
    #              '''
    sql_getall = '''
                select mac_id,index_value,pdate FROM u_ads_wh.v_opn_washer_water_rep_mon where prod_group_cd ='FA' and index_code ='01060B0104'
                '''
#零冷水
#     sql_getall = '''select mac_id,index_value,pdate FROM u_ads_wh.v_opn_washer_water_rep_mon where prod_group_cd ='FB' and index_code ='0118OB0104'
...
#                    '''
    ...
    counts = temp.drop_duplicates().groupby('pdate')['index_value'].value_counts().to_frame('count').reset_index()
